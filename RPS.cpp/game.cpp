...
Game::Game(int rounds, GameMode mode)
    : totalRounds(rounds), currentRound(0), player1Score(0), player2Score(0), gameMode(mode) {}
...
void Game::displayStats() const {
    ...
    std::cout << "Final Score: Player 1 - " << player1Score << ", Player 2 - " << player2Score << std::endl;
    std::cout << getResult() << std::endl;
}

Move Game::getRandomMove() const {
    std::vector<Move> moves = { Move::ROCK, Move::PAPER, Move::SCISSORS };
...
void Game::requestDraw() {
    std::cout << "Request draw functionality not implemented.\n";
}

void Game::forfeit() {
    std::cout << "Forfeit functionality not implemented.\n";
}
